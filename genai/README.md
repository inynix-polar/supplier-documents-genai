...
Из корня репозитория можно собрать минимальный runtime, который без ключей запускает
детерминированный eval:

```bash
docker build --file genai/Dockerfile --tag supplier-documents-genai:eval .
docker run --rm supplier-documents-genai:eval
docker run --rm supplier-documents-genai:eval --seed 17
```

Тесты изолированы в отдельном target и не попадают в runtime-образ:
...
Оба образа используют `LLM_FAKE=1`. Runtime запускается без root и содержит только
production-зависимости, `app/` и `eval/`.
...
- multi-stage Dockerfile разделяет воспроизводимый test target и минимальный eval runtime.
...
- [x] **Базовый CI/CD** — GitHub Actions: линтер + `pytest` (с `LLM_FAKE=1`) на push/PR.
- [x] **Docker** — `Dockerfile`, прогоняющий эвал/тесты в режиме `LLM_FAKE=1` без ключей.
- [x] **Управление зависимостями** — стартовый `requirements.txt` заменён на
  `pyproject.toml` + воспроизводимый `uv.lock`; параллельный Poetry не нужен.
- [x] **Тесты** — на грунтинг-фильтр (отбраковка галлюцинации) и на устойчивость парсинга.
